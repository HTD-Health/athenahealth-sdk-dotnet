...
namespace AthenaHealth.Sdk.Tests.EndToEnd.Clients.Patient
{
    public class PatientClientTests
    {
        ...
        {
            Should.NotThrow(async () => await Client.Patients.SetDefaultPharmacy(195900, 5000, new SetPharmacyRequest{DepartmentId = 164, ClinicalProviderId = 11242674}));
        }

        [Fact]
        public void AddPreferredPharmacy_ValidData_NotThrow()
        {
            Should.NotThrow(async () => await Client.Patients.AddPreferredPharmacy(195900, 5000, new SetPharmacyRequest{DepartmentId = 164, ClinicalProviderId = 11242674}));
        }
    }
}
